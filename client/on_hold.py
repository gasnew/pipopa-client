  ...
  def play(self):
    try:
      if self.paused:
        mixer.music.unpause()
        self.paused = False
        print(' [HOLD] Resuming on-hold music!')
      elif not mixer.music.get_busy():
        if self.rad_countdown == 0:
          name = 'rad/{}'.format(random.choice(filenames('hold/rad')))
          self.rad_countdown = self.RAD_PERIOD
        else:
          if (self.play_count + 2) % self.INFO_PERIOD == 0:
            name = 'info/arabesque.ogg'
          elif (self.play_count + 2) % self.INFO_PERIOD == 1:
            name = 'info/stego_urls.ogg'
          else:
